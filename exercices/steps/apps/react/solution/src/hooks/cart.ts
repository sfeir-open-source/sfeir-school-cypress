...
import { CartDTO, getCart, addToCart as add, removeFromCart as remove, payCart as pay, UserPaymentDTO } from '../api/cart'
...
export default function useCart (): Cart {
  ...
  const payCart = useCallback((userPaymentDTO: UserPaymentDTO) => {
    return pay(userPaymentDTO)
      .then(() => setCart(DEFAULT_CART))
  }, [])
